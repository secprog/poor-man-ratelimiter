...
- `filter/ApiPortFilter.java`: blocks admin API calls on the public gateway port (8080).
...
- `utils/websocket.js`: real-time analytics client, browser-safe (connects to `/api/ws/analytics`).
...
- Default ports: gateway `8080`, admin API `9090`, frontend `3000`, redis `6379`, test-server `9000`.

Anti-bot challenges:
- Meta-refresh (HTML only), JS token, and Preact challenge that sets `X-Form-Token-Challenge` cookie then reloads after configurable seconds (`antibot-preact-difficulty`).

Rate limit identifiers:
- Priority is Header > Cookie > Body field > JWT claims > IP.
- Body field extraction supports JSON, form URL-encoded, XML, and multipart bodies via `BodyFieldExtractor`.
